...
func (h *HTTPServer) serveCached(w http.ResponseWriter, r *http.Request, hvf *HVFile) {
	...
	if writeErr != nil {
		Debug("serve interrupted", "bytes", humanBytes(sent), "expected", humanBytes(hvf.Size), "duration", time.Since(started).String(), "path", r.RequestURI, "err", writeErr)
		return
	}
	Info("served", "code", 200, "bytes", humanBytes(sent), "expected", humanBytes(hvf.Size), "duration", time.Since(started).String(), "path", r.RequestURI)
}
...
func (h *HTTPServer) proxyFile(w http.ResponseWriter, r *http.Request, hvf *HVFile, fileindex, xres, fileid string) bool {
	...
	_ = chosen
	if writeErr != nil {
		Debug("proxy response interrupted", "bytes", humanBytes(sent), "expected", humanBytes(n), "duration", time.Since(started).String(), "path", r.RequestURI, "err", writeErr)
		return cached
	}
	Info("proxied", "code", 200, "bytes", humanBytes(sent), "expected", humanBytes(n), "duration", time.Since(started).String(), "path", r.RequestURI)
	return cached
}
